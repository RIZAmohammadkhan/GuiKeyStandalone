...
pub fn perform_generation(app_state: &mut GeneratorAppState) -> Result<(), GeneratorError> {
    ...
    let bootstrap_addrs_for_client_config: Vec<String> = app_state.bootstrap_addresses_str.split(',')
    ...
    if bootstrap_addrs_for_client_config.is_empty() {
        ...
        return Err(GeneratorError::InputValidation {
            field: "Bootstrap Multiaddresses".to_string(),
            message: "No valid bootstrap multiaddresses found after parsing (must start with '/').".to_string(),
        });
    }
    ...
    if app_state.server_config.web_ui_listen_address.split(':').count() != 2 {
        app_state.operation_in_progress = false;
        return Err(GeneratorError::InputValidation {
    ...
    app_state.generated_key_hex_display_snippet = encryption_key_hex.chars().take(8).collect::<String>() + "...";
    ...
    let server_libp2p_keypair = Keypair::ed25519_from_bytes(server_identity_seed_bytes)
        .map_err(|e| GeneratorError::Other(format!("Failed to create libp2p keypair from seed bytes: {:?}", e)))?;
    ...
    app_state.status_message = format!("Configuration data prepared. Server PeerID: {}", server_peer_id);
    ...
    // --- Create README ---
    let local_server_ui_access_address = app_state.server_config.web_ui_listen_address.replace("0.0.0.0", "127.0.0.1");
    ...
    let readme_content = format!(
        ...
        server_seed_snippet = app_state.server_config.server_identity_key_seed_hex.chars().take(16).collect::<String>(),
