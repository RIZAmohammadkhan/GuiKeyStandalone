...
fn init_server_diagnostics(log_level_str: &str) {
    ...
    tracing_subscriber::fmt()
        ...
        .init();
    tracing::info!(
        "Server: Diagnostic logging initialized. Effective filter: '{}'",
        filter_description_for_log
    );
}
...
async fn main() -> std::io::Result<()> {
    ...
    let settings = match ServerSettings::new() {
        ...
        Err(e) => {
            ...
            eprintln!(
                "FATAL: Server configuration error: {}. Ensure 'local_server_config.toml' exists and is valid.",
                e
            );
    ...
    tracing::info!(
        "Server: Configured P2P Listen Multiaddr: '{}'",
        settings.p2p_listen_address
    );
    tracing::info!(
        "Server: Configured Web UI Listen Address: '{}'",
        settings.web_ui_listen_address
    );
    tracing::debug!(
        "Server: DB Path='{:?}', Log Retention: {} days",
