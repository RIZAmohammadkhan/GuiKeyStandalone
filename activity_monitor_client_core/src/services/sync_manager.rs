                        ...
                        Ok(serialized_data) => {
                            ...
                            match encrypt_payload(&serialized_data, &settings.encryption_key) {
                                Ok(encrypted_app_payload) => {
                                    let mut attempts = 0;
                                    loop { // Retry loop for sending this specific batch via P2P
                                        attempts += 1;
                                        tracing::debug!("SyncManager: Attempting to send batch (attempt {}/{}) via P2P.", attempts, settings.max_retries_per_batch);

                                        // 3. Send via P2pDataSender
                                        // The app_client_id (UUID) is taken from settings.
                                        match p2p_data_sender.send_log_batch(
                                            ...
                                            Ok(log_batch_response) => {
                                                if log_batch_response.status == "success" {
                                                    tracing::info!(
                                                        ...
                                                        batch_size, attempts, log_batch_response.events_processed, log_batch_response.message
                                                    );
                                                    ...
                                                    tracing::error!(
                                                        ...
                                                        log_batch_response.status, log_batch_response.message, attempts
                                                    );
                                            ...
                                            Err(e) => { // Network-level or P2P internal error from P2pDataSender
                                                tracing::warn!(
                                ...
                                Err(e) => {
                                    tracing::error!("SyncManager: Failed to encrypt batch for P2P sending: {}. Batch will be retried later.", e);
